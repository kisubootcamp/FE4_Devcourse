...
export default function ScrollComparison() {
  ...
  useEffect(() => {
    const el = document.querySelector("#refEffect")!;
    el.scrollTo({ top: el.scrollHeight }); //해도되지만 최대한 가상DOM 사용위해 ref객체 활용
  ...
  return (
    <div className="space-y-8 p-8 max-w-2xl mx-auto">
      ...
      <div>
        ...
        <div id="refEffect" ref={refEffect} className={boxClass}>
          {items.map((text) => (
      ...
      <div>
        ...
        <div id="refLayout" ref={refLayout} className={boxClass}>
          {items.map((text) => (
            <div key={text} className="py-1">
              {text}
            </div>
