...
export default function Stopwatch() {
  const [time, setTime] = useState(0); // 0ms
  const [isRunning, setIsRunning] = useState(false); // true 시작, false 정지
  //undefined=>false되므로 useEffect가 변경사항으로 보고 실행됨
  // ms
  ...
  useEffect(() => {
    ...
    return () => {
      if (intervalId) clearInterval(intervalId); //변경될때마다 useEffect실행=>setInterval도 실행=>중복실행,누수 발생할 수 있음
    };
  }, [isRunning]);
  return (
