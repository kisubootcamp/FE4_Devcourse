...
export default function UseState() {
  ...
  //...setting이 왜필요하고 theme은 왜 setting의 속성이라 할 수 있는건지?
  //=> useState로 setting은 theme속성이 있는 객체임을 선언해놓았음.
  // 배열 데이터
  ...
  const addUserHandler = () => {
    setUsers((users) => [...users, { id: 4, name: "Minji" }]); //id=4 객체를 배열에 추가
  };
  const deleteUserHandler = () => {
    setUsers((users) => users.filter((user) => user.id !== 4)); //id=4 객체를 배열에서 삭제
  };
