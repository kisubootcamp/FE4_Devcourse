...
const fetchMovie = async (url: string) => {
  await new Promise((resolve) =>
    setTimeout(resolve, [3000, 4000, 5000, 6000, 7000][Math.random() * 5])
  );
...
export default function Movie() {
  return (
    <>
      ...
      <ErrorBoundary fallback={<MovieError title="Up Coming" />}>
        <Suspense fallback={<MovieLoader title="Up Coming" />}>
          <MovieList
