...
1.2.2 emotion 
-> CSS를 런타임에 생성한다.
-> 단점 : 초기 로딩에 대한 오버헤드가 크다

1.2.3 Vanilla Extract 
-> 타입스크립트를 완벽하게 지원 + 제로 런타임
...
1.4 그외
1.4.1 이미지 렌더링하기
public 폴더 -> 정적 리소스(파비콘, robots.txt, 구글 애널리틱스 인증파일, 네이버 콘솔 인증 등등..)
src 폴더 -> 동적 리소스 (대부분의 이미지는 src폴더에 넣는 것이 좋음.)
번들링 -> 리소스를 최적화 (압축, 캐시)
1.4.2 폰트 적용하기 
*/

// 버전 숫자에 대한 의미 (ex 14.5.1)
