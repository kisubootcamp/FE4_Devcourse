...
// sumAll 함수는 rest parameter를 사용하여 배열의 길이에 관계없이 숫자들을 받을 수 있어야 합니다. 함수 호출 후 결과를 출력해보세요.
{
    // 함수 정의
    const sumAll = (...nums: number[]): number => nums.reduce((sum, num) => sum + num, 0); // 0은 sum의 초기값을 의미합니다.
...
// a는 필수 매개변수로 받고, b는 기본값을 0으로 설정합니다. 이 함수에 타입을 지정하고 호출해보세요.
{
    // 함수 정의
...
// message가 없으면 기본값 "Welcome"을 사용해야 합니다. 이 함수의 타입을 지정하고, 호출해보세요.
{
    // 함수 정의
