// 연습 문제
// 1. 객체 병합
// 두 개의 객체를 병합하여 반환하는 함수를 작성하시오
// 중복된 키가 있을 경우, 두 번째 객체의 값이 우선한다.
{
...
// 3. 객체 값 필터링
// 주어진 객체에서 값이 특정 기준 이상인 키-값 쌍만 필터링 하여 반환하는 함수 작성
{
  const filterByThreshold = (obj, hold) => {
    const result = { ...obj };
    for (const key in result) {
      if (result[key] < hold) delete result[key];
    }
...
// 주어진 객체의 모든 값을 제곱하는 함수를 작성하세요.
{
  const squareValues = (obj) => {
    const result = { ...obj };
    for (const key in result) result[key] **= 2;
...
// 주어진 객체에서 중복된 값을 제거하고, 그에 해당하는 키만 반환하는 함수를 작성하세요.
{
  const removeDuplicateValues = (obj) => {
    ...
    for (const key in obj) {
      if (count[obj[key]] === 1) result[key] = obj[key];
    }
    return result;
  };
...
// 7. 객체의 값으로 키 찾기
// 주어진 객체에서 특정 값에 해당하는 키를 반환하는 함수를 작성하세요. 값이 여러 개일 경우, 첫 번째 키만 반환한다. 값이 없으면 null 반환
{
  const findKeyByValue = (obj, findKey) => {
    for (const key in obj) if (obj[key] === findKey) return key;
    return null;
  };
