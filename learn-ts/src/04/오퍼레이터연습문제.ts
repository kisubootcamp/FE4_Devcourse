{
  ...
  function printValue(a: number | string): void {
    console.log(a);
  }
...
{
  //2. **유니언 타입을 사용한 조건 처리**
  //**문제:** 주어진 입력 값이 `string`이면 그 길이를, `number`이면 2배 값을 반환하는 함수 `doubleOrLength`를 작성하세요.
  // 함수는 `string` 타입이면 문자열의 길이를, `number` 타입이면 그 숫자의 두 배 값을 반환해야 합니다
  function doubleOrLength(value: string | number): string | number {
...
{
  // 4. **유니언 타입을 사용한 배열 처리**
  //**문제:** 배열이 `number[]` 또는 `string[]`일 때, 배열의 첫 번째 요소를 반환하는 함수 `getFirstElement`를 작성하세요.
  // 배열이 비어 있을 경우 `undefined`를 반환해야 합니다. 반환값은 배열의 첫 번째 요소 타입과 동일해야 합니다.
  function getFirstElement(arr: number[] | string[] | null[]): number | string | undefined {
...
{
  //5. **유니언 타입을 사용한 값 비교**
  //**문제:** 두 값을 받아서 두 값이 같으면 `true`를, 다르면 `false`를 반환하는 함수 `isEqual`을 작성하세요.
  // 입력 값은 `string` 또는 `number`일 수 있으며, 두 값이 동일하면 `true`를 반환하고, 다르면 `false`를 반환해야 합니다.
  function isEqual(value1: number | string, value2: number | string): boolean {
...
}
{
  ...
  //**문제:** 두 값을 받아서 그 중 더 큰 값을 반환하는 함수 `maxValue`를 작성하세요.
  // 입력 값은 `number` 또는 `string`일 수 있으며, 숫자일 경우 그대로 반환하고, 문자열일 경우 길이가 더 긴 문자열을 반환합니다. 만약 두 값이 같으면 그 값을 반환하세요.
  function maxValue(v1: number | string, v2: number | string): number | string {
...
}
{
  ...
  // **문제:**`name`, `age` 속성을 포함하는 객체와 `email` 속성을 포함하는 객체를 합쳐서 새로운 객체를 반환하는 `createContact` 함수를 작성하세요.
