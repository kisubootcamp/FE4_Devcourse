//제네릭->타입을 변수로 받을 수 있기 때문에 타입의 재사용성이 증가 (간단하게 타입 지정 가능), 코드의 재사용성이 증가
//1.<T,U>->최대 2개(), 3개 <T,U,K>
...
const r5 = returnTuple(true, { name: "sucoding" });
//K:Key-> 객체 타입의 키
//V:Value->객체 타입의 값
//제네릭 타입 제약 조건
function getValue<K extends keyof V, V>(obj: V, key: K): V[K] {
  return obj[key]; //<>속 K,V 순서와 함수 매개변수 순서는 관련 없음 오히려 <K,V>(V,K) 처럼 사용함
}
