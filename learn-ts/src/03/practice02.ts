{
  ...
  // **문제 설명**: `sumAllNumbers` 함수는 **여러 개의 숫자 배열**을 인자로 받아
  // 각 배열의 합을 구하여 반환하는 함수입니다.
  // `rest parameter`와 `map`을 사용하여 여러 배열을 처리하고 결과를 반환해야 합니다.
  // 함수 정의
  const sumAllNumbers = (...num: number[][]): number[] => {
    return num.map((arr) => arr.reduce((acc, cur) => acc + cur, 0));
  };
  // 함수 호출
  ...
  // **문제 설명**: `multiplyArrays` 함수는 두 개의 **숫자 배열**을 받아
  // 각 배열에서 해당 인덱스의 숫자를 곱한 값을 새로운 배열로 반환합니다.
  // 배열의 길이가 다르면 짧은 배열에 맞춰서 곱셈을 해야 합니다.
  // 함수 정의
  const multiplyArrays = (x: number[], y: number[]): number[] => {
    const result3: number[] = [];
    if (x.length === y.length || x.length < y.length) {
      for (let i = 0; i < x.length; i++) {
        result3.push(x[i] * y[i]);
      }
  ...
  // 함수 호출
  const result4 = multiplyArrays([1, 2, 3], [4, 5, 6]);
  console.log(result4); // [4, 10, 18]

  // ### **문제 4: 두 개의 배열을 받아 교차된 값을 반환하는 함수**
  // **문제 설명**: `intersection` 함수는
  // 두 개의 **배열**을 받아 교차된 값들만 반환하는 함수입니다.
  // 두 배열에서 겹치는 값들을 찾아 반환해보세요.
  // 함수 정의
  const intersection = (x: number[], y: number[]): number[] => {
  ...
  // **문제 설명**: `formatInfo` 함수는 **객체**를 받아,
  // `name` 필드가 존재하면 `"Name: {name}"`을 출력하고,
  // `age` 필드가 존재하면 `"Age: {age}"`를 출력합니다.
  // 매개변수 타입을 지정하고 호출해보세요.
  // 함수 정의
  const formatInfo = (infoObj: {
    name?: string;
    age?: string;
  }): string | undefined => {
    if (infoObj.name !== undefined) {
      return `Name: ${infoObj.name}`;
    } else if (infoObj.age !== undefined) {
      return `Age: ${infoObj.age}`;
    }
  };
  // 함수 호출
  ...
  // **문제 설명**: `combineStrings` 함수는 두 개의 **문자열**을
  // 받아 합쳐서 반환하는 함수입니다. 두 문자열의 순서를 바꿀 수 있도록 하며,
  ...
  // **문제 설명**: `squareArray` 함수는 **숫자 배열**을 받아
  // 그 배열의 각 값에 대해 제곱을 계산하여 새로운 배열을 반환합니다.
  // 함수 정의
  const squareArray = (num: number[]): number[] => {
    return num.map((n) => n ** 2);
  };
  // 함수 호출
  const result6 = squareArray([1, 2, 3, 4]);
  console.log(result6); // [1, 4, 9, 16]

  // ### **문제 8: 문자열과 숫자가 주어졌을 때, 그 둘을 더하여 문자열로 반환하는 함수**
  // **문제 설명**: `addStringAndNumber` 함수는 **문자열**과 **숫자**를 인자로 받아
  // 두 값을 더하여 문자열로 반환하는 함수입니다.
  // 적절한 타입을 지정하여 함수를 정의하고 호출해보세요.
  // 함수 정의
  const addStringAndNumber = (message: string, num: number): string => {
    return message + num;
  };
  // 함수 호출
  const result7 = addStringAndNumber("The number is ", 10);
  console.log(result7); // "The number is 10"
  ...
  // 함수 정의
  const compareValues = (num: number, text: string): string => {
    if (text.length > num) {
  ...
  // 함수 호출
  const result8 = compareValues(10, "Hello");
  console.log(result8); // "Number is larger"
