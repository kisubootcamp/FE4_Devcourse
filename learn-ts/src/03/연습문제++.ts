...
}
{
  ...
  const doubleValue = function (value: string | number): string | number {
...
}
{
  ...
  function formatInfo(userInfo: { name?: string; age?: number }): string {
    if (userInfo.name) return `"Name: ${userInfo.name}"`;
    else return `Age: ${userInfo.age}`;
  }
  // 함수 호출
...
}
{
  ...
  // 함수 정의
  function compareValues(a: number, b: string): void {
    if (a > b.length) return console.log("Number is larger");
    if (b.length > a) return console.log("String is larger");
  }
