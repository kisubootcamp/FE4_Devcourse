...
}
{
  ...
  const doubleOrLength = (value: string | number) => {
    return typeof value === "string" ? value.length * 2 : value * 2;
  };
...
}
{
  ...
  console.log(mergedObject); // 예상 출력: { name: "Alice", age: 30, jobTitle: "Engineer", salary: 5000 }
}
{
  // 4. 유니언 타입을 사용한 배열 처리
...
}
{
  ...
  console.log(updatedPerson); // 예상 출력: { name: "Jane", age: 28, address: "123 Maple St" }

  const updatedPersonWithoutAddress = updateAddress({ name: "John", age: 22 });
  console.log(updatedPersonWithoutAddress); // 예상 출력: { name: "John", age: 22 }
}
{
...
}
{
  ...
  console.log(personWithContact); // 예상 출력: { name: "Alice", age: 28, email: "alice@example.com" }
}
{
