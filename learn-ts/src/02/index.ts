// let num: number = 10;
// 타입스크립트는 다른 파일에 있는 코드여도 하나의 파일처럼 인식하고 검사한다. 그렇기 때문에 1일차 ts파일에서 num이 먼저 선언되었기 때문에 여기서는 num이 중복되어 생성될 수 없다.
// 각각의 파일 안에서 중괄호를 사용(코드 블럭)해서 문제를 해결할 수도 있다.
// 코드 블럭을 사용하면 다른 파일의 변수 선언과 중복되어도 사용 가능하다.
// 블럭 스코프를 지정함으로 스코프 오염을 막은 것.
{
  let num: number = 10;
}

{
  ...
  // truthy / falsy는 완전히 boolean이 아니고 JS에서 자동으로 형변환을 해서 사용하기 때문에 boolean 타입에서 직접적으로 사용할 수 없다 -> JS가 자동으로 평가를 내릴 때만 true. false의 느낌으로 사용된다.
  ...
  // 최종 코드에는 any와 unknown이 절대 있으면 안된다. 꼭 써야 한다면 any보다는 unknown을 사용하는게 더 낫다
  ...
  let numArr2: Array<number> = [10, 20]; // 이제는 더 이상 쓰지 않는 코드
  ...
  mixArr.push("A"); // 타입 추론 -> 타입스크립트가 타입을 자동으로 추론해주는 기능 지정된 타입 이외는 넣을 수 없음
