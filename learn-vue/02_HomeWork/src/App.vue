<template>
  <div class="saramin">
    ...
    <div class="box">
      ...
      <div class="str-info">
        <p>
          공백 포함 <span>{{ str.length }}</span> 자 | <span>{{ getByte(str) }}</span> byte
        </p>
        <p>
          공백 제외 <span>{{ str.replace(/\s/g, '').length }}</span> 자 |
          <span>{{ getByte(str.replace(/\s/g, '')) }}</span> byte
        </p>
      </div>
      <div class="btn-area">
