<script setup></script>
<template>
  <header class="header">
    <h1 class="header__logo">Wave</h1>
    <ul class="header__navi">
      <li>
        <a href="#">홈</a>
      </li>
      <li>
        <a href="#">마테고리</a>
      </li>
      <li>
        <a href="#">Movie</a>
      </li>
      <li>
        <a href="#">Tv</a>
      </li>
    </ul>
    <div class="header-search">
      <input type="text" class="header-search__input" placeholder="제목으로 찾아보세요." />
      <span class="material-symbols-outlined icon"> search </span>
    </div>
  </header>
  <nav class="navigator">
    <ul class="navigator-list">
      <li class="navigator-list__item">
        <img src="assets/icons/home.png" alt="" class="navigator-list__icon" />
        <strong class="navigator-list__txt">홈</strong>
      </li>
      <li class="navigator-list__item">
        <img src="assets/icons/live.png" alt="" class="navigator-list__icon" />
        <strong class="navigator-list__txt">상영중</strong>
      </li>
      <li class="navigator-list__item">
        <img src="assets/icons/video.png" alt="" class="navigator-list__icon" />
        <strong class="navigator-list__txt">개봉예정</strong>
      </li>
      <li class="navigator-list__item">
        <img src="assets/icons/search.png" alt="" class="navigator-list__icon" />
        <strong class="navigator-list__txt">검색</strong>
      </li>
      <li class="navigator-list__item">
        <img src="assets/icons/popluar.png" alt="" class="navigator-list__icon" />
        <strong class="navigator-list__txt">인기콘텐츠</strong>
      </li>
    </ul>
  </nav>
  <section class="movie list">
    <h4 class="movie-subtext">현재 상영중인 영화</h4>
    <div class="movie-list">
      <div class="movie-list__item">
        <a href="#" class="skeleton-list-item ui0"></a>
      </div>
      <div class="movie-list__item">
        <a href="#">
          <figure>
            <img src="https://image.tmdb.org/t/p/w300/tlcuhdNMKNGEVpGqBZrAaOOf1A6.jpg" alt="" />
          </figure>
          <div class="movie-list__txt">
            <div class="progress-circle p50">
              <span>10%</span>
              <div class="left-half-clipper">
                <div class="first50-bar"></div>
                <div class="value-bar"></div>
              </div>
            </div>
            <strong class="movie-list__title">Civil War</strong>
            <p class="movie-list__desc">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatibus dolore alias
              quas porro voluptas debitis repudiandae similique odio corporis magnam ipsam ex fugit
              eligendi autem, officiis, culpa dolorem laborum adipisci.
            </p>
            <span class="movie-list__release">2023.11.21 / 평점 7.3</span>
          </div>
        </a>
      </div>
      <div class="movie-list__item">
        <a href="#">
          <figure>
            <img src="https://image.tmdb.org/t/p/w300/tlcuhdNMKNGEVpGqBZrAaOOf1A6.jpg" alt="" />
          </figure>
          <div class="movie-list__txt">
            <div class="progress-circle p50">
              <span>10%</span>
              <div class="left-half-clipper">
                <div class="first50-bar"></div>
                <div class="value-bar"></div>
              </div>
            </div>
            <strong class="movie-list__title">Civil War</strong>
            <span class="movie-list__release">2023.11.21 / 평점 7.3</span>
          </div>
        </a>
      </div>
      <div class="movie-list__item">
        <a href="#">
          <figure>
            <img src="https://image.tmdb.org/t/p/w300/tlcuhdNMKNGEVpGqBZrAaOOf1A6.jpg" alt="" />
          </figure>
          <div class="movie-list__txt">
            <div class="progress-circle p50">
              <span>10%</span>
              <div class="left-half-clipper">
                <div class="first50-bar"></div>
                <div class="value-bar"></div>
              </div>
            </div>
            <strong class="movie-list__title">Civil War</strong>
            <p class="movie-list__desc">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatibus dolore alias
              quas porro voluptas debitis repudiandae similique odio corporis magnam ipsam ex fugit
              eligendi autem, officiis, culpa dolorem laborum adipisci.
            </p>
            <span class="movie-list__release">2023.11.21 / 평점 7.3</span>
          </div>
        </a>
      </div>
      <div class="movie-list__item">
        <a href="#">
          <figure>
            <img src="https://image.tmdb.org/t/p/w300/tlcuhdNMKNGEVpGqBZrAaOOf1A6.jpg" alt="" />
          </figure>
          <div class="movie-list__txt">
            <div class="progress-circle p50">
              <span>10%</span>
              <div class="left-half-clipper">
                <div class="first50-bar"></div>
                <div class="value-bar"></div>
              </div>
            </div>
            <strong class="movie-list__title">Civil War</strong>
            <p class="movie-list__desc">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatibus dolore alias
              quas porro voluptas debitis repudiandae similique odio corporis magnam ipsam ex fugit
              eligendi autem, officiis, culpa dolorem laborum adipisci.
            </p>
            <span class="movie-list__release">2023.11.21 / 평점 7.3</span>
          </div>
        </a>
      </div>
      <div class="movie-list__item">
        <a href="#">
          <figure>
            <img src="https://image.tmdb.org/t/p/w300/tlcuhdNMKNGEVpGqBZrAaOOf1A6.jpg" alt="" />
          </figure>
          <div class="movie-list__txt">
            <div class="progress-circle p50">
              <span>10%</span>
              <div class="left-half-clipper">
                <div class="first50-bar"></div>
                <div class="value-bar"></div>
              </div>
            </div>
            <strong class="movie-list__title">Civil War</strong>
            <p class="movie-list__desc">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatibus dolore alias
              quas porro voluptas debitis repudiandae similique odio corporis magnam ipsam ex fugit
              eligendi autem, officiis, culpa dolorem laborum adipisci.
            </p>
            <span class="movie-list__release">2023.11.21 / 평점 7.3</span>
          </div>
        </a>
      </div>
      <div class="movie-list__item">
        <a href="#">
          <figure>
            <img src="https://image.tmdb.org/t/p/w300/tlcuhdNMKNGEVpGqBZrAaOOf1A6.jpg" alt="" />
          </figure>
          <div class="movie-list__txt">
            <div class="progress-circle p50">
              <span>10%</span>
              <div class="left-half-clipper">
                <div class="first50-bar"></div>
                <div class="value-bar"></div>
              </div>
            </div>
            <strong class="movie-list__title">Civil War</strong>
            <p class="movie-list__desc">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatibus dolore alias
              quas porro voluptas debitis repudiandae similique odio corporis magnam ipsam ex fugit
              eligendi autem, officiis, culpa dolorem laborum adipisci.
            </p>
            <span class="movie-list__release">2023.11.21 / 평점 7.3</span>
          </div>
        </a>
      </div>
    </div>
  </section>
</template>
<style scoped></style>
