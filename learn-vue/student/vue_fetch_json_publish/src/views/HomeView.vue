...
const loadPosts = async () => {
  ...
  try {
    ...
    userMap.value = Object.fromEntries(users.map((user) => [user.id, user]))
  } catch (err) {
...
<template>
  <div class="home-view">
    <div class="container">
      <!-- 검색 후 결과 노출 영역 -->
      <div class="search-results mb-3" v-if="searchQuery">
        ...
        <!-- 데이터를 찾았을 때 -->
        <p v-else>Found {{ filteredPosts.length }} posts</p>
      </div>
      ...
      <div class="posts-grid" v-else>
        <div class="post-item" v-for="post in filteredPosts" :key="post.id">
          <PostCard :post="post" :user="userMap[post.userId]" />
        </div>
      </div>
    </div>
  </div>
