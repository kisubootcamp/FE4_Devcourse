<template>
  <div class="saramin">
    ...
    <div class="box">
      ...
      <div class="str-info">
        <p>
          공백 포함 <span>{{ text.length }}</span> 자 | <span>{{ getByte(text) }}</span> byte
        </p>
        <p>
          공백 제외 <span>{{ text.replace(/\s/g, '').length }}</span> 자 |
          <span>{{ getByte(text.replace(/\s/g, '')) }}</span> byte
        </p>
      </div>
      <div class="btn-area">
        <button @click="textCopy">전체복사</button>
        <button @click="textReset">초기화</button>
      </div>
    </div>
  </div>
</template>
