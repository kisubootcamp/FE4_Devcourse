...
const searchQuery = computed(() => (route.query.serach ? route.query.search : ''))
...
const loadPosts = async () => {
  ...
  try {
    ...
    error.value = 'Failed to load posts. Please try agian.'
    console.log(err)
  } finally {
    loading.value = false
  }
  ...
  <div class="home-view">
    <div class="container">
      ...
      <div v-if="searchQuery" class="search-results mb-3">
        ...
        <!-- 검색된 데이터가 없을 때 -->
        <p v-if="!loadPosts && filteredPosts.length === 0">
          No posts found for your search. Try a different query.
        </p>
        <!-- 데이터를 찾았을 때 -->
        <p v-else-if="!loading">Found {{ filteredPosts.length }}</p>
      </div>
