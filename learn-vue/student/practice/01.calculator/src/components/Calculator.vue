<template>
  <div class="calculator">
    ...
    <button class="operation" @click="selectOperation('x')">x</button>
...
export default {
  ...
  methods: {
    ...
    calculator() {
      ...
      const prev = parseFloat(this.previousOperand) * 10000000;
      const current = parseFloat(this.currentOperand) * 10000000; // "A"
      if (isNaN(prev) || isNaN(current)) return;

      switch (this.operation) {
        case '+':
          computation = (prev + current) / 10000000; //1e8
          break;
        case '-':
          computation = (prev - current) / 10000000;
          break;
        case 'x':
          computation = (prev * current) / Math.pow(10000000, 2);
          break;
        case '/':
