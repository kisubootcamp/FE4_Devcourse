...
export default {
  ...
  computed: {
    // 화면에 표시할 현재 숫자 문자열
    currText() {
      if (this.currOper === "") return "0";
      else return this.formatNumber(this.currOper);
    },
    prevText() {
      if (!this.prevOper && !this.operation) return "";
      return `${this.formatNumber(this.prevOper)} ${this.operation}`;
    },
  },
  methods: {
    // 숫자 자판을 클릭했을 때
    ...
    calculator() {
      ...
      const prev = parseFloat(this.prevOper) * 100000000;
      const curr = parseFloat(this.currOper) * 100000000;
      ...
      switch (this.operation) {
        ...
        case "-":
          ...
          break;
        case "×":
          computation = (prev * curr) / Math.pow(100000000, 2);
          break;
        case "÷":
      ...
      this.currOper = computation.toString();
      this.operation = null;
      this.prevOper = "";
    },
    // 입력된 숫자 삭제
...
<template>
  <div class="calculator">
    <div class="display">
      <div class="previous-operand">{{ prevText }}</div>
      <div class="current-operand">{{ currText }}</div>
    </div>
