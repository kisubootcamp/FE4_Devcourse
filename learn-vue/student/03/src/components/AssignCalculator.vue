<template>
  <div class="calculator">
    ...
    <button class="operation" @click="selectOperation('x')">×</button>
...
<script>
export default {
  name: 'AssignCalculator',

  data() {
    return {
      currentOperand: '', // 현재 입력 중인 숫자
  ...
  methods: {
    ...
    calculator() {
      ...
      const current = parseFloat(this.currentOperand) * 100000000
...
export default {
  ...
  methods: {
    clickNum(value) {
      ...
      this.current = Number(this.expression1)
    },
    clickOperation(ex) {
      //operation 따로 저장
      if (ex === this.operations.includes('+')) {
      }
    },
    clickAc() {
      this.expression1 = ''
  ...
  // computed: {
  //   calculateSum() {
  //     const result = this.expression1 + this.expression2
  //     return result
  //   },
  // },
}
...
<template>
  <div class="calculator">
    <div class="display">
      <div class="previous-operand">{{ expression1 }}</div>
      <div class="current-operand">{{ current }}</div>
    </div>
    ...
    <button class="operation" @click="clickOperation(' + ')">+</button>
    <button class="span-two" @click="clickNum(0)">0</button>
