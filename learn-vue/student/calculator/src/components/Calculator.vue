<template>
  <div class="calculator">
    <div class="display">
      <div class="previous-operand">{{ previous.join(' ') }}</div>
      <div class="current-operand">{{ current ? current : 0 }}</div>
    </div>
    <button class="span-two" @click="clickReset">AC</button>
    <button @click="clickDelete">DEL</button>
    ...
    <button class="operation equals" @click="clickResult">=</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    clickNumber(num) {
      if (!this.current && num === 0) return;
      if (!this.current && num === '.') num = '0.';
      this.current += num;
    },
    clickOperation(op) {
      this.previous.push(this.current + ' ' + op);
      this.current = '';
    },
    clickResult() {
      ...
      for (const pre of this.previous) {
        if (!previoustNum) {
          previoustNum = pre.substr(0, pre.length - 2);
          previoustOp = pre[pre.length - 1];
          continue;
        }
